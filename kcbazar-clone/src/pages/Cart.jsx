                      ...
                      <tr key={item.id} className="group hover:bg-gray-50/50 transition-colors">
                        <td className="py-6 px-6">
                          <div className="flex items-center gap-4">
                            ...
                            <Link to={`/product/${item.id}`} className="text-sm font-bold text-[#111] leading-tight max-w-[250px] hover:text-[#FF4D6D] transition-colors">{item.name}</Link>
                          </div>
                        </td>
                        ...
                        <td className="py-6 px-4">
                          <div className="flex items-center border border-gray-200 rounded-lg w-fit bg-white h-[44px]">
                            <button onClick={() => updateQuantity(item.id, -1)} className="w-10 h-full flex items-center justify-center text-gray-400 hover:text-[#FF4D6D] cursor-pointer">-</button>
                            <span className="w-10 text-center font-bold text-[#111]">{item.quantity}</span>
                            <button onClick={() => updateQuantity(item.id, 1)} className="w-10 h-full flex items-center justify-center text-gray-400 hover:text-[#FF4D6D] cursor-pointer">+</button>
                          </div>
                        </td>
                        <td className="py-6 px-6 text-sm font-black text-[#FF4D6D]">৳ {item.price * item.quantity}</td>
                      </tr>
                    ))}
                    {cartItems.length === 0 && (
                        <tr>
                            <td colSpan="4" className="py-20 text-center">
                                ...
                                <Link to="/shop" className="inline-block py-3 px-8 bg-[#FF4D6D] text-white font-black uppercase text-xs tracking-widest rounded-lg">Return To Shop</Link>
                            </td>
                        </tr>
                    )}
                  </tbody>
          ...
          <div className="w-full lg:w-[400px] flex flex-col gap-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-black/5 p-8">
               ...
               <div className="space-y-6">
                 ...
                 <Link to="/checkout" className="w-full h-[56px] bg-[#FF4D6D] text-white flex items-center justify-center font-black uppercase tracking-[2px] rounded-xl text-sm shadow-xl shadow-[#FF4D6D]/20 hover:bg-[#e64462] transition-all active:scale-[0.98] cursor-pointer mt-4">Proceed To Checkout</Link>
               </div>
            </div>
                    ...
                    {deliveryFaqs.map((faq, i) => (
                        <div key={i} className="border border-gray-50 rounded-lg overflow-hidden">
                            <button 
                                ...
                                className="w-full flex items-center justify-between p-4 text-left font-bold text-[13px] text-[#111] hover:bg-gray-50 transition-colors"
                            >
                                <span className={openFaq === i ? 'text-[#FF4D6D]' : ''}>{faq.question}</span>
                                <svg className={`w-4 h-4 transition-transform duration-300 ${openFaq === i ? 'rotate-180 text-[#FF4D6D]' : 'text-gray-300'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3"><path d="M19 9l-7 7-7-7"/></svg>
                            </button>
                            {openFaq === i && (
