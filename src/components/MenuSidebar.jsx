      ...
      <div className={`fixed inset-y-0 left-0 z-[3001] w-full max-w-[320px] bg-white shadow-2xl transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full relative overflow-hidden">
          ...
          <div className="p-6 bg-gray-50 border-t border-gray-100">
            ...
            <p className="text-[11px] text-gray-400 font-medium">© {new Date().getFullYear()} {settings?.siteName || settings?.siteTitle?.split('|')[0]?.trim() || 'Mailbon'} | Premium Beauty & Wellness. All rights reserved.</p>
          </div>
        </div>
      </div>
    </>
