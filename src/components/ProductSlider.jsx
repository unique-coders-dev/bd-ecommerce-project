            ...
            <div key={product.id} className="h-full">
              <Link href={product.link} className="bg-white border border-[#efefef] rounded-xl overflow-hidden transition-all duration-300 flex flex-col hover:-translate-y-1.5 hover:shadow-xl group relative h-full">
                  <div className="relative aspect-square bg-[#f9f9f9] overflow-hidden">
                    <span className="absolute top-2.5 left-2.5 bg-primary-dark text-white text-[11px] font-bold px-2 py-1 rounded z-[2] shadow-sm">{product.discount}</span>
                    ...
                    <button className="absolute bottom-0 left-0 w-full bg-primary text-white py-3 font-bold text-sm opacity-0 translate-y-full transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0 z-[5] hover:bg-primary-soft hover:text-primary active:bg-primary-dark">
                      Add to cart
                    </button>
                    ...
                    <div className="absolute top-2.5 right-2.5 flex flex-col gap-2 opacity-0 group-hover:opacity-100 translate-x-2.5 transition-all duration-300 z-10">
                      <button className="w-9 h-9 rounded-full bg-white border border-[#eee] flex items-center justify-center text-[#333] transition-all hover:bg-primary-soft hover:text-primary shadow-md">
                ...
                <div className="p-4 flex flex-col gap-1.5 flex-grow">
                  ...
                  <h3 className="text-sm font-semibold text-[#333] h-10 overflow-hidden line-clamp-2 leading-tight group-hover:text-[var(--primary-color)] transition-colors">{product.name}</h3>
