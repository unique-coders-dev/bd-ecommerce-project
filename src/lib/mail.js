...
export const sendAdminOtpEmail = async (email, name, otp) => {
  const settings = await prisma.setting.findUnique({
    where: { id: "site-settings" },
  });
  ...
  const mailOptions = {
    from: `"${siteName} Admin Security" <${process.env.EMAIL_FROM}>`,
    to: email,
    subject: `Your ${siteName} Admin Login Code`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e1e1e1; border-radius: 10px;">
        <div style="text-align: center; margin-bottom: 30px;">
           <img src="${logoUrl}" alt="${siteName}" style="height: 50px;">
        </div>
        <h2 style="color: #111; text-align: center;">Admin Verification Code</h2>
        <p style="color: #666; line-height: 1.6;">Hello ${name},</p>
        <p style="color: #666; line-height: 1.6;">Use the following One-Time Password (OTP) to log in to your ${siteName} admin dashboard. This code will expire in 10 minutes.</p>
        <div style="text-align: center; margin: 30px 0;">
          <div style="background-color: #f8f9fa; border: 2px dashed ${primaryColor}; color: #111; padding: 20px; text-decoration: none; border-radius: 8px; font-weight: 900; font-size: 32px; letter-spacing: 5px; display: inline-block;">
            ${otp}
          </div>
        </div>
        <p style="color: #999; font-size: 12px; line-height: 1.6;">If you didn't request this code, please secure your account immediately.</p>
        <hr style="border: 0; border-top: 1px solid #eee; margin: 30px 0;">
        <p style="color: #ccc; font-size: 11px; text-align: center;">&copy; ${year} ${siteName}. All rights reserved.</p>
      </div>
    `,
