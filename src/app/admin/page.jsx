...
const AdminOverview = () => {
  const stats = [
    ...
    { label: 'Conversion Rate', value: '3.2%', trend: '-0.4%', color: 'text-red-500', bg: 'bg-red-50' },
  ];
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-black/5 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden group">
         <div className="absolute top-0 right-0 w-64 h-64 bg-[#FF4D6D]/5 rounded-full -mr-32 -mt-32 transition-transform group-hover:scale-110"></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
         {stats.map((stat, i) => (
            <div key={i} className="bg-white p-6 rounded-2xl shadow-sm border border-black/5 flex flex-col gap-4 group hover:border-[#FF4D6D]/10 transition-all">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="xl:col-span-2 bg-white p-8 rounded-3xl shadow-sm border border-black/5">
             <div className="flex justify-between items-center mb-8">
                ...
                <div className="flex gap-2">
                    <button className="px-3 py-1 text-[10px] font-black uppercase bg-gray-50 text-gray-400 rounded-lg underline-offset-4 decoration-[#FF4D6D]">Weekly</button>
             ...
             <div className="h-[300px] w-full flex items-end gap-2 px-4 border-b border-l border-gray-100 relative">
                {[45, 60, 40, 85, 55, 90, 70].map((h, i) => (
                    <div key={i} className="flex-1 bg-[#FF4D6D]/10 hover:bg-[#FF4D6D] transition-all rounded-t-lg relative group" style={{ height: `${h}%` }}>
                ...
                <div className="absolute inset-0 flex flex-col justify-between pointer-events-none opacity-20 pr-4">
                    {[1, 2, 3, 4].map(i => <div key={i} className="w-full border-t border-dashed border-[#FF4D6D]"></div>)}
                </div>
             </div>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-black/5 flex flex-col">
             <div className="flex justify-between items-center mb-8">
                ...
                <button className="text-[10px] font-black text-[#FF4D6D] uppercase tracking-widest hover:underline cursor-pointer">View All</button>
             </div>
             <div className="space-y-6 flex-1">
                {recentOrders.map((order, i) => (
                    <div key={i} className="flex items-center justify-between group">
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center font-black text-[10px] text-gray-400 group-hover:bg-[#FF4D6D]/10 group-hover:text-[#FF4D6D] transition-all">
                                {order.customer[0]}
                            </div>
             ...
             <div className="mt-8 pt-8 border-t border-gray-50 flex flex-col gap-4">
                ...
                <div className="w-full h-2 bg-gray-50 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-[#FF4D6D] to-[#ff7d94]" style={{ width: '82%' }}></div>
                </div>
             </div>
          </div>
